const ensureElement = window.BASTION_DOM?.get || function(id, tagName = "div", defaults = {}) {
  ...
  Object.entries(defaults).forEach(([key, value]) => element[key] = value);
  return element;
};

const analyzeBtn = ensureElement("analyzeBtn", "button");
const analysisPanel = ensureElement("analysisPanel", "div");
const filtersPanel = ensureElement("filtersPanel", "div");
const unitFilter = ensureElement("unitFilter", "select");
const longRangeOnly = ensureElement("longRangeOnly", "input", { type: "checkbox" });
const searchFilter = ensureElement("searchFilter", "input", { type: "text", value: "" });
const resetFiltersBtn = ensureElement("resetFiltersBtn", "button");
...
const filterStatus = ensureElement("filterStatus", "div");
const lowBalanceThreshold = ensureElement("lowBalanceThreshold", "input", { type: "number", value: "10" });
const stockFilter = ensureElement("stockFilter", "select", { value: "all" });
const sortFilter = ensureElement("sortFilter", "select", { value: "default" });
const actionPriorityFilter = document.getElementById("actionPriorityFilter") || ensureElement("actionPriorityFilter", "select", { value: "all" });
const actionLongOnly = document.getElementById("actionLongOnly") || ensureElement("actionLongOnly", "input", { type: "checkbox" });
const actionStatusFilter = document.getElementById("actionStatusFilter") || ensureElement("actionStatusFilter", "select", { value: "all" });
...
window.BASTION_DOM?.on(analyzeBtn, "click", analyzeWorkbook);
window.BASTION_DOM?.on(unitFilter, "change", applyFilters);
window.BASTION_DOM?.on(longRangeOnly, "change", applyFilters);
window.BASTION_DOM?.on(searchFilter, "input", applyFilters);
window.BASTION_DOM?.on(resetFiltersBtn, "click", resetFilters);
...
window.BASTION_DOM?.on(lowBalanceThreshold, "input", applyFilters);
window.BASTION_DOM?.on(stockFilter, "change", applyFilters);
window.BASTION_DOM?.on(sortFilter, "change", applyFilters);
